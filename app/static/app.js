...
async function runChat(){
  const text=promptEl.value.trim();if(!text||busy)return;if(contextAttachmentPending()){setContextOpen(true);toast("Hãy đợi tệp xử lý xong trước khi gửi.","error");return}if($("#context").value.length>100000){setContextOpen(true);toast("Ngữ cảnh vượt giới hạn 100.000 ký tự.","error");return}const provider=$("#provider").value,model=selectedModel(),mode=selectedMode(),status=selectionStatus(),submittedContextSources=contextSourcesForRequest();if(provider!=="fake"&&status==="missing"){toast(`${providerText(provider)} chưa có khóa API. Chọn provider khác hoặc cấu hình .env.`,"error");return}
...
function clearContextFile(){clearDraftAttachments();persistedContextSources=[]}
