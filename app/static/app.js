...
let rawEvents = [], compareBusy = false, currentMode = null, liveTurn = null, currentRunEvidence = null, currentMetrics = {};
...
const SUPPORTED_CONTEXT_EXTENSIONS = Object.freeze(["txt","md","json","csv"]);

// Normal chat has one truthful strategy contract: Adaptive AUTO. Runtime mode
// is evidence emitted by the controller, never a user-forced baseline choice.
function mapUiModeToChatStrategy(value){return String(value||"auto").toLowerCase()==="auto"?"adaptive-auto":null}
...
function modeText(value){return UI_TEXT.modes[String(value||"")]||String(value||"—")}
...
function renderModelPicker(){if(!cfg)return;const provider=$("#provider").value,model=selectedModel(),providers=$("#providerChoices"),models=$("#modelChoices");if(!providers||!models)return;
...
function populateModels(preferred=null){
  ...
  items.forEach(item=>{const o=document.createElement("option");o.value=item.id;o.textContent=item.label;o.dataset.tier=item.tier;select.appendChild(o)});if(items.some(x=>x.id===target))select.value=target;select.disabled=items.length<=1;updateProviderDisplay()
}
...
function setTurnAnswer(card,text,meta={}){
  ...
  summary.innerHTML=`<b>${esc(`Tự động thích ứng → ${modeText(mode)}`)}</b>${details.length?` · ${esc(details.join(" · "))}`:""}${meta.stopReason?` · <span class="run-stop">${esc(stopText(meta.stopReason))}</span>`:""}`;
...
function renderSnapshot(meta){
  ...
  const docs=(meta.source_document_ids||[]).join(", ")||"(không có)";
...
function resetInspector(mode="running"){
  trace.innerHTML=mode==="idle"?'<div class="empty">Chọn một lượt chạy trong hội thoại để xem luồng thực thi.</div>':"";rawEvents=[];currentRunEvidence=null;currentMetrics={};$("#rawEvents").textContent="[]";$("#insMode").textContent="—";currentMode=null;$("#modeName").textContent="Tự động";resetSnapshot();
...
function appendTrace(e){
  ...
  if(e.meta?.mode){currentMode=e.meta.mode;$("#insMode").textContent=modeText(currentMode);$("#modeName").textContent=`Tự động → ${modeText(currentMode)}`}
  if(e.title==="AUTO route selected"){const agents=e.meta?.selected_agents||{};$("#autoExplain").innerHTML=`<b>Tự động chọn → ${esc(modeText(e.detail))}</b><span>${esc(phraseText(e.meta?.why||e.detail||""))} · ${Object.entries(agents).filter(x=>x[1]).map(x=>roleText(x[0])+" × "+x[1]).join(" · ")}</span>`}
}
...
function evidenceModel(){
  ...
  const model={run,events,analysis:null,mode:run.mode||null,why:"",selection:{},planEvents:[],batches:[],escalations:[],verifications:[],stop:null,agents:[],metrics:Object.keys(currentMetrics||{}).length?currentMetrics:(run.metrics||{})};
  ...
  for(const event of events){
    ...
    if(event.title==="AUTO route selected"){model.mode=event.detail||meta.mode||model.mode;model.why=meta.why||""}
...
function renderQuickDetails(model){
  ...
  const retrieval=run.retrieval_meta||{},chunks=Array.isArray(retrieval.selected_chunks)?retrieval.selected_chunks:[],docs=Array.isArray(retrieval.source_document_ids)?retrieval.source_document_ids:[],$sources=$("#quickSources");
...
async function runChat(){
  const text=promptEl.value.trim();if(!text||busy)return;const provider=$("#provider").value,model=selectedModel(),status=selectionStatus();if(provider!=="fake"&&status==="missing"){toast(`${providerText(provider)} chưa có khóa API. Chọn provider khác hoặc cấu hình .env.`,"error");return}
  busy=true;sendBtn.disabled=true;currentRunId=null;$("#exportBtn").disabled=true;liveTurn=makeTurnCard(text,{pending:true});history.push({role:"user",content:text});promptEl.value="";autoSize();resetInspector();currentRunEvidence={strategy:"adaptive",provider,model,events:rawEvents,status:"running",stop_reason:""};$("#insStrategy").textContent=UI_TEXT.labels.adaptiveAuto;$("#insProvider").textContent=providerText(provider);renderEvidencePanels();
  try{
    const response=await fetch("/api/chat/stream",{method:"POST",headers:{"Content-Type":"application/json"},body:JSON.stringify({message:text,context:$("#context").value,provider,model,conversation_id:currentConversationId,history:history.slice(0,-1)})});if(!response.ok)throw new Error(await response.text());if(!response.body)throw new Error("Server không trả về stream");
...
async function loadConversation(id){
  if(busy)return;try{const response=await fetch(`/api/conversations/${encodeURIComponent(id)}`);if(!response.ok)throw new Error("not found");const conversation=await response.json();currentConversationId=id;rememberConversation(id);app.classList.remove("mobile-side-open");updatePanelButtons();history=(conversation.messages||[]).map(m=>({role:m.role,content:m.content}));messages.innerHTML="";const turns=conversation.turns||fallbackTurns(conversation.messages);for(const turn of turns)renderTurn(turn,conversation);$("#chatTitle").textContent=conversation.title||"Cuộc trò chuyện";if(typeof conversation.context==="string")$("#context").value=conversation.context;if(conversation.provider&&cfg.available[conversation.provider]){$("#provider").value=conversation.provider;populateModels(conversation.model)}currentRunId=(conversation.run_ids||[]).at(-1)||null;$("#exportBtn").disabled=!currentRunId;await loadConversations();if(currentRunId)await loadRunInspector(currentRunId);else resetInspector("idle");scrollBottom()
...
async function loadRunInspector(id){
  try{const response=await fetch(`/api/runs/${encodeURIComponent(id)}`);if(!response.ok)throw new Error("not found");const run=await response.json();currentRunId=id;$("#exportBtn").disabled=false;rawEvents=run.events||[];currentRunEvidence={...run,events:rawEvents};currentMetrics=run.metrics||{};trace.innerHTML="";$("#rawEvents").textContent=JSON.stringify(safeFrontendEvidence(run),null,2);currentMode=null;renderSnapshot(run.retrieval_meta);rawEvents.forEach(appendTrace);renderMetrics(run.metrics);$("#insStrategy").textContent=run.strategy==="adaptive"?UI_TEXT.labels.adaptiveAuto:strategyText(run.strategy);$("#insProvider").textContent=providerText(run.provider);$("#runState").textContent=stopText(run.stop_reason||run.status);$("#runState").className="run-state "+(run.status==="failed"?"error":run.status==="degraded"?"warning":"done");renderEvidencePanels();$$('.turn-card').forEach(card=>card.classList.toggle('selected-run',card.dataset.runId===id))
...
function setFileChipState(state,file,message=""){
  const chip=$("#chatFileChip"),status=chip.querySelector(".file-status");chip.className=`file-chip show ${state==="error"||state==="unsupported"?"error":""}`;chip.querySelector(".file-name").textContent=file?.name||"Tệp";status.className=`file-status${state==="error"||state==="unsupported"?" error":""}`;status.textContent=message||({loading:"Đang tải...",processing:"Đang xử lý...",ready:"Sẵn sàng",error:"Không thể xử lý",unsupported:"Không hỗ trợ định dạng tệp"}[state]||state)
}
async function processContextFile(file){
  activeContextFile=file;const ext=(file?.name?.split(".").pop()||"").toLowerCase();setFileChipState("loading",file);await Promise.resolve();if(!SUPPORTED_CONTEXT_EXTENSIONS.includes(ext)){setFileChipState("unsupported",file);return false}setFileChipState("processing",file);
  try{const text=await file.text();if(text.length>100000)throw new Error("Tệp vượt giới hạn 100.000 ký tự");$("#context").value=text;setFileChipState("ready",file);toast(`Đã nạp ${file.name} vào ngữ cảnh`,"success");return true}catch(error){setFileChipState("error",file,error.message||"Không thể xử lý");return false}
}
function clearContextFile(){activeContextFile=null;const chip=$("#chatFileChip");chip.className="file-chip";chip.querySelector(".file-name").textContent="";chip.querySelector(".file-status").textContent="";$("#contextFile").value=""}
...
$("#attachButton").onclick=()=>$("#contextFile").click();$("#closeContext").onclick=()=>setContextOpen(false,true);$("#clearContext").onclick=()=>{$("#context").value="";clearContextFile();toast("Đã xóa ngữ cảnh")};$("#contextFile").onchange=async event=>{const file=event.target.files[0];if(file)await processContextFile(file);event.target.value=""};$("#chatFileChip .file-retry").onclick=()=>{if(activeContextFile)processContextFile(activeContextFile)};$("#chatFileChip .file-remove").onclick=()=>{$("#context").value="";clearContextFile()};
...
$("#helpButton").onclick=()=>toggleAnchoredPopover($("#helpButton"),$("#helpPopover"));$$('[data-help]').forEach(button=>button.onclick=()=>{const text={usage:"Gửi câu hỏi; Adaptive AUTO sẽ tự chọn cách xử lý.",files:"Hỗ trợ TXT, Markdown, JSON và CSV dạng văn bản.",modes:"DIRECT, PARALLEL và PLANNED là kết quả điều phối runtime.",about:"Adaptive Agent là ứng dụng local dùng FastAPI và bằng chứng thực thi thật."}[button.dataset.help];toast(text)});$("#shareButton").onclick=()=>toggleAnchoredPopover($("#shareButton"),$("#sharePopover"));$("#profile").onclick=()=>toggleAnchoredPopover($("#profile"),$("#accountMenu"));
