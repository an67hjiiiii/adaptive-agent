...
function rebuildContextFromAttachments(){
  ...
  $("#context").value=ready.map(item=>`===== ${safeContextFilename(contextSourceName(item.source),"Tệp")} =====\n${item.text}`).join("\n\n--- attached file ---\n\n");
}
...
async function processContextFile(file,id=null){
  ...
  try{
    ...
    item.status="ready";item.text=prepared.text;item.source=prepared.source;item.filename=safeContextFilename(prepared.source.filename||item.filename,file.name||"Tệp");item.error="";rebuildContextFromAttachments();renderContextAttachments();toast(`Đã nạp ${file.name} vào ngữ cảnh`,"success");return true
  }catch(error){
...
function clearDraftAttachments(){activeContextFile=null;contextAttachments.clear();const input=$("#contextFile");if(input)input.value="";const list=$("#contextFileList");if(list)list.innerHTML=""}
...
$("#attachButton").onclick=()=>$("#contextFile").click();$("#contextFile").onchange=async event=>{const files=[...event.target.files];if(files.length){$("#context").value="";await Promise.all(files.map(file=>processContextFile(file)))}event.target.value=""};
