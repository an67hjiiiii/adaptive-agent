...
let conversationCache = [], pendingDeleteConversation = null, pendingRenameConversation = null, activeContextFile = null, conversationLoadSequence = 0;
let contextAttachments = new Map(), contextAttachmentSequence = 0, persistedContextSources = [], draftContextActive = false, projectWorkspace = null;
...
const PROJECT_NOISE_DIRECTORIES = new Set([".git","node_modules","dist","build","__pycache__",".venv","venv","coverage",".next"]);
...
const UPSTREAM_ERROR_MESSAGES = Object.freeze({
  502:"Máy chủ tạm thời không phản hồi. Hãy thử lại sau vài giây.",
  503:"Dịch vụ hiện tạm thời không khả dụng. Hãy thử lại sau.",
  504:"Yêu cầu mất quá nhiều thời gian phản hồi. Hãy thử lại.",
  generic:"Không thể kết nối tới dịch vụ. Vui lòng thử lại.",
});
...
function markdownTableRow(value){
  const raw=String(value ?? "").trim();if(!raw.includes("|"))return [];
  const body=raw.startsWith("|")?raw.slice(1):raw,trimmed=body.endsWith("|")?body.slice(0,-1):body;
  const cells=[];let cell="";
  for(let i=0;i<trimmed.length;i++){
    const character=trimmed[i];
    if(character==="|"){cells.push(cell.trim());cell="";continue}
    if(character==="\\"&&trimmed[i+1]==="|"){cell+="|";i++;continue}
    cell+=character;
  }
  cells.push(cell.trim());return cells;
}
function markdownTableDivider(value){const cells=markdownTableRow(value);return cells.length>1&&cells.every(cell=>/^:?-{3,}:?$/.test(cell))}
function renderMarkdownTable(header,rows){
  const width=Math.max(header.length,...rows.map(row=>row.length),1),normalize=cells=>Array.from({length:width},(_,index)=>cells[index]||"");
  return `<div class="answer-table-wrap" role="region" aria-label="Bảng dữ liệu" tabindex="0"><table><thead><tr>${normalize(header).map(cell=>`<th scope="col">${inline(cell)}</th>`).join("")}</tr></thead><tbody>${rows.map(row=>`<tr>${normalize(row).map(cell=>`<td>${inline(cell)}</td>`).join("")}`).join("")}</tbody></table></div>`;
}
function markdown(value){
  ...
  for(let i=0;i<lines.length;i++){const line=lines[i];
    ...
    const header=markdownTableRow(line),divider=lines[i+1];
    if(header.length>1&&divider!==undefined&&markdownTableDivider(divider)){
      flush();closeList();const rows=[];i++;
      while(i+1<lines.length){const row=markdownTableRow(lines[i+1]);if(row.length<2||markdownTableDivider(lines[i+1]))break;rows.push(row);i++}
      html+=renderMarkdownTable(header,rows);continue;
    }
...
function upstreamErrorMessage(status){return UPSTREAM_ERROR_MESSAGES[Number(status)]||UPSTREAM_ERROR_MESSAGES.generic}
function isUnsafeErrorPage(value,contentType=""){
  const raw=String(value??"").trim(),lower=raw.toLowerCase(),type=String(contentType??"").toLowerCase();
  if(type.includes("text/html"))return true;
  if(/^\s*<!doctype\s+html(?:\s|>)/i.test(raw)||/^\s*<html(?:\s|>)/i.test(raw))return true;
  if(/<head(?:\s|>)[\s\S]*<body(?:\s|>)/i.test(raw))return true;
  if(/<(?:html|head|body)(?:\s|>)/i.test(raw)&&/<\/(?:html|body)\s*>/i.test(raw))return true;
  return /(?:^|\n)\s*traceback \(most recent call last\):/i.test(raw)
    || (raw.length>240&&/(?:^|\n)\s*at\s+\S+/i.test(raw)&&/(?:error|exception|stack)/i.test(lower));
}
async function responseError(response){
  const error=new Error(await response.text());error.status=response.status;error.contentType=response.headers?.get?.("content-type")||"";return error
}
function safeErrorDetail(value,status=null,contentType=""){
  const raw=String(value??"").trim();let detail=raw,structured=false;
  try{const parsed=JSON.parse(raw),candidate=parsed?.detail??parsed?.error;structured=Boolean(parsed&&typeof parsed==="object");detail=typeof candidate==="object"&&candidate!==null?String(candidate.message??candidate.detail??raw):String(candidate||raw)}catch{}
  if(isUnsafeErrorPage(raw,contentType)||isUnsafeErrorPage(detail,contentType))return upstreamErrorMessage(status);
...
function attachmentEntries(sources){
  const seen=new Set();return (Array.isArray(sources)?sources:[]).map(source=>{
    const preferred=typeof source==="object"?(source.relative_path||source.relativePath||source.safe_path||source.filename||source.file_name||source.name):source;
    const fallback=typeof source==="object"?(source.filename||source.file_name||source.name):"Tệp";
    const label=safeContextFilename(preferred,safeContextFilename(fallback,"Tệp"));if(!label||seen.has(label))return null;seen.add(label);return {label}
  }).filter(Boolean)
}
function renderUserAttachments(sources){
  const entries=attachmentEntries(sources);if(!entries.length)return "";
  return `<div class="turn-attachments" aria-label="Tệp đính kèm"><span class="source-label">Tệp đính kèm</span><div class="turn-attachment-list">${entries.map(entry=>`<span class="turn-attachment-chip">${svgIcon("file","small")}${esc(entry.label)}</span>`).join("")}</div></div>`
}
function metricNumberText(value){
  if(value===null||value===undefined||value==="")return "";
  const number=Number(value);return Number.isFinite(number)?number.toLocaleString("en-US"):""
}
function compactMetric(value){
  const number=Number(value);if(value===null||value===undefined||value===""||!Number.isFinite(number))return "—";
  const absolute=Math.abs(number),trimmed=value=>value.toFixed(1).replace(/\.0$/,"" );
  if(absolute>=1000000)return `${trimmed(number/1000000)}m`;
  if(absolute>=1000)return `${trimmed(number/1000)}k`;
  return number.toLocaleString("en-US")
}
function latencyMetric(value){
  if(value===null||value===undefined||value==="")return "—";
  const number=Number(value);if(!Number.isFinite(number))return "—";
  return number>=1000?`${(number/1000).toFixed(1).replace(".",",")}s`:`${number}ms`
}
function metricTooltip(value,suffix){const text=metricNumberText(value);return text?`${text} ${suffix}`:`Không có dữ liệu · ${suffix}`}
function latencyTooltip(value){if(value===null||value===undefined||value==="")return "Thời gian xử lý: Không có dữ liệu";const number=Number(value);if(!Number.isFinite(number))return "Thời gian xử lý: Không có dữ liệu";return number>=1000?`Thời gian xử lý: ${(number/1000).toFixed(1)} giây`:`Thời gian xử lý: ${number} mili giây`}
function runMetricMarkup(icon,value,tooltip){const display=icon==="clock-3"?latencyMetric(value):compactMetric(value);return `<span class="run-metric" role="img" aria-label="${esc(tooltip)}" title="${esc(tooltip)}">${svgIcon(icon,"small")}<span class="run-metric-value">${esc(display)}</span></span>`}
function runModeMarkup(mode){
  const normalized=mapUiModeToChatStrategy(mode)||String(mode||"AUTO").toUpperCase(),icon=normalized==="PARALLEL"?"git-fork":normalized==="PLANNED"?"workflow":"route",label=modeText(normalized),tooltip=`Chế độ xử lý: ${label}`;
  return `<span class="run-metric run-mode-metric" role="img" aria-label="${esc(tooltip)}" title="${esc(tooltip)}">${svgIcon(icon,"small")}<span class="run-mode-label">${esc(label)}</span></span>`
}
function makeTurnCard(userText,{createdAt=null,pending=false,attachments=[]}={}){
  clearWelcome();const card=document.createElement("article");card.className="turn-card";
  card.innerHTML=`<div class="turn-question"><div class="turn-label"><b>Bạn</b><span class="turn-time">${esc(fmtTime(createdAt))}</span></div><div class="question-text">${esc(userText)}</div>${renderUserAttachments(attachments)}</div><div class="turn-answer"><div class="answer-head"><div class="avatar">${svgIcon("agent")}</div><div class="answer-head-copy"><b>Adaptive Agent</b><span class="answer-meta">${UI_TEXT.labels.adaptiveAuto}</span></div><span class="mode-pill">${pending?"Đang xử lý":modeText("AUTO")}</span></div><div class="answer-body">${pending?'<div class="answer-pending"><span class="pulse"></span><span>Đang phân tích task và điều phối Agent…</span></div>':""}</div><div class="turn-footer"></div></div>`;
...
function setTurnAnswer(card,text,meta={}){
  ...
  summary.innerHTML=`<span class="run-summary-metrics">${runModeMarkup(mode)}${runMetricMarkup("users",m.agent_executions,metricTooltip(m.agent_executions,"Agent tham gia"))}${runMetricMarkup("refresh-cw",m.logical_calls,metricTooltip(m.logical_calls,"lượt gọi mô hình"))}${runMetricMarkup("binary",m.total_tokens,metricTooltip(m.total_tokens,"token"))}${runMetricMarkup("clock-3",m.e2e_ms,latencyTooltip(m.e2e_ms))}${showStop&&meta.stopReason?`<span class="run-stop">${esc(stopText(meta.stopReason))}</span>`:""}</span>`;
  const entries=sourceEntries(meta.sources,meta.contextSources),uploaded=entries.filter(entry=>entry.uploaded),other=entries.filter(entry=>!entry.uploaded),sourceGroup=(label,items)=>`<div class="turn-source-group"><span class="source-label">${label}</span>${items.map(entry=>`<span class="source-chip">${svgIcon("file","small")}${esc(entry.label)}</span>`).join("")}</div>`;
  if(entries.length){const sourceBox=document.createElement("div");sourceBox.className="turn-sources";sourceBox.innerHTML=`${uploaded.length?sourceGroup("Tệp đã dùng",uploaded):""}${other.length?sourceGroup("Nguồn",other):""}`;footer.appendChild(sourceBox)}
  const copy=document.createElement("button");copy.className="mini-action footer-icon-action";copy.type="button";copy.innerHTML=svgIcon("copy","small");copy.setAttribute("aria-label","Sao chép");copy.title="Sao chép";copy.onclick=()=>navigator.clipboard.writeText(text||"").then(()=>toast("Đã sao chép","success"));actions.appendChild(copy);
  if(meta.runId){const run=document.createElement("button");run.className="run-pill";run.classList.add("footer-icon-action");run.type="button";run.innerHTML=svgIcon("circle-info","small");run.setAttribute("aria-label","Chi tiết xử lý");run.title="Chi tiết xử lý";run.onclick=()=>{openInspector();loadRunInspector(meta.runId)};actions.appendChild(run);const compare=document.createElement("button");compare.className="mini-action footer-icon-action compare-turn";compare.type="button";compare.innerHTML=svgIcon("git-compare-arrows","small");compare.setAttribute("aria-label","So sánh kết quả");compare.title="So sánh kết quả";compare.onclick=openCompare;actions.appendChild(compare);card.dataset.runId=meta.runId}
  if(meta.status==="failed"){
    const retry=document.createElement("button");retry.className="mini-action footer-icon-action retry-turn";retry.type="button";retry.innerHTML=svgIcon("refresh-cw","small");retry.setAttribute("aria-label","Thử lại câu hỏi của lượt chạy thất bại");retry.title="Thử lại";retry.onclick=()=>{const original=card.querySelector(".question-text")?.textContent?.trim();if(!original||busy)return;promptEl.value=original;autoSize();runChat()};actions.appendChild(retry)
  }
  const footerRow=document.createElement("div");footerRow.className="turn-footer-row";footerRow.append(actions,summary);footer.appendChild(footerRow)
}
function renderTurn(turn,conversation,isLatest=false){
  const user=turn.user||{},assistant=turn.assistant||{},storedSources=[user.context_sources,user.attachments,user.files,turn.context_sources,turn.attachments].find(item=>Array.isArray(item)&&item.length)||(isLatest&&Array.isArray(conversation.context_sources)?conversation.context_sources:[]);const card=makeTurnCard(user.content||"(Không có câu hỏi)",{createdAt:user.created_at,attachments:storedSources});setTurnAnswer(card,assistant.content||"(Chưa có câu trả lời)",{runId:assistant.run_id,provider:assistant.provider||conversation.provider,model:assistant.model||conversation.model,status:assistant.status,stopReason:assistant.stop_reason,mode:assistant.mode,requestedMode:assistant.requested_mode||assistant.processing_mode||conversation.processing_mode,metrics:assistant.metrics,sources:assistant.sources||assistant.context_sources||[],contextSources:conversation.context_sources||[]});return card
}
...
function renderQuickDetails(model){
  ...
  const retrieval=run.retrieval_meta||{},chunks=Array.isArray(retrieval.selected_chunks)?retrieval.selected_chunks:[],attached=Array.isArray(retrieval.attached_sources)?retrieval.attached_sources:[],docs=(attached.length?attached.map(source=>safeContextFilename(sourceFilename(source),"Tệp")).filter(Boolean):Array.isArray(retrieval.source_document_ids)?retrieval.source_document_ids:[]),$sources=$("#quickSources"),sourceHeading=$sources?.closest(".quick-sources")?.querySelector("h3");
  if(sourceHeading)sourceHeading.textContent=attached.length?"Tệp đã dùng":"Nguồn";
...
async function runChat(){
  const text=promptEl.value.trim();if(!text||busy)return;if(contextAttachmentPending()){setContextOpen(true);toast("Hãy đợi tệp xử lý xong trước khi gửi.","error");return}if($("#context").value.length>100000){setContextOpen(true);toast("Ngữ cảnh vượt giới hạn 100.000 ký tự.","error");return}const provider=$("#provider").value,model=selectedModel(),mode=selectedMode(),status=selectionStatus(),activeContext=activeContextForRequest(),submittedContextSources=contextSourcesForRequest();if(provider!=="fake"&&status==="missing"){toast(`${providerText(provider)} chưa có khóa API. Chọn provider khác hoặc cấu hình .env.`,"error");return}
  currentRequestedMode=mode;busy=true;sendBtn.disabled=true;currentRunId=null;$("#exportBtn").disabled=true;liveTurn=makeTurnCard(text,{pending:true,attachments:submittedContextSources});history.push({role:"user",content:text,context_sources:submittedContextSources});setContextOpen(false);promptEl.value="";autoSize();resetInspector();currentRequestedMode=mode;currentRunEvidence={strategy:"adaptive",provider,model,processing_mode:mode,events:rawEvents,status:"running",stop_reason:""};$("#insStrategy").textContent=mode==="adaptive-auto"?UI_TEXT.labels.adaptiveAuto:modeText(mode);$("#insProvider").textContent=providerText(provider);renderEvidencePanels();
  try{
    const response=await fetch("/api/chat/stream",{method:"POST",headers:{"Content-Type":"application/json"},body:JSON.stringify({message:text,context:activeContext?$("#context").value:"",context_active:activeContext,context_sources:submittedContextSources,provider,model,mode,conversation_id:currentConversationId,history:history.slice(0,-1)})});if(!response.ok)throw await responseError(response);if(!response.body)throw new Error("Server không trả về stream");
    ...
    currentConversationId=final.conversation_id||currentConversationId;rememberConversation(currentConversationId);currentRunId=final.run_id||null;$("#exportBtn").disabled=!currentRunId;const answer=final.answer?.trim()||(final.status==="failed"?friendlyRunError(final.error,final.provider||provider,final.model||model,final.http_status,final.content_type):"Lượt chạy kết thúc nhưng chưa tạo được câu trả lời.");const completedMode=mapUiModeToChatStrategy(final.processing_mode||mode)||mode;currentRunEvidence={...(currentRunEvidence||{}),run_id:currentRunId,strategy:"adaptive",provider:final.provider||provider,model:final.model||model,processing_mode:completedMode,status:final.status,stop_reason:final.stop_reason,sources:final.sources||[],events:rawEvents,metrics:final.metrics||currentMetrics};setTurnAnswer(liveTurn,answer,{runId:currentRunId,provider:final.provider,model:final.model,status:final.status,stopReason:final.stop_reason,mode:currentMode||completedMode,requestedMode:completedMode,metrics:final.metrics,sources:final.sources||[],contextSources:submittedContextSources});if(final.status!=="failed")clearDraftAttachments();history.push({role:"assistant",content:answer,sources:final.sources||[]});$("#chatTitle").textContent=history.find(x=>x.role==="user")?.content.slice(0,54)||"Adaptive Agent";$("#runState").textContent=stopText(final.stop_reason||final.status||"completed");$("#runState").className="run-state "+(final.status==="failed"?"error":final.status==="degraded"?"warning":"done");renderEvidencePanels();if(final.status==="failed")toast("Không thể hoàn thành lượt chạy.","error");if(!updateConversationSidebar(currentConversationId,text,{runId:currentRunId}))void loadConversations()
  }catch(error){
    if(!error.persisted&&history.at(-1)?.role==="user"&&history.at(-1)?.content===text)history.pop();const fatal=error.fatal||{};const answer=friendlyRunError(error.message,fatal.provider||provider,fatal.model||model,error.status||fatal.http_status,error.contentType||fatal.content_type);currentRunEvidence={...(currentRunEvidence||{}),run_id:fatal.run_id||currentRunId,provider:fatal.provider||provider,model:fatal.model||model,processing_mode:mapUiModeToChatStrategy(fatal.processing_mode||mode)||mode,status:"failed",stop_reason:fatal.stop_reason||"STOP_FAILURE",events:rawEvents,metrics:{}};setTurnAnswer(liveTurn,answer,{runId:fatal.run_id,provider:fatal.provider||provider,model:fatal.model||model,status:"failed",stopReason:fatal.stop_reason||"STOP_FAILURE",mode:currentMode||mode,requestedMode:mode,metrics:{},contextSources:submittedContextSources});$("#runState").textContent=stopText(fatal.stop_reason||"STOP_FAILURE");$("#runState").className="run-state error";renderEvidencePanels();toast("Không thể hoàn thành lượt chạy.","error");if(error.persisted&&!updateConversationSidebar(currentConversationId,text,{runId:fatal.run_id||currentRunId}))void loadConversations()
...
async function testProvider(){
  ...
  try{const response=await fetch("/api/provider/test",{method:"POST",headers:{"Content-Type":"application/json"},body:JSON.stringify({provider,model})});if(!response.ok)throw await responseError(response);const data=await response.json();const ok=data.error_category==="SUCCESS",status=ok?"ready":data.error_category==="NOT_CONFIGURED"?"missing":"failed";cfg.provider_status[provider]={status,model,error_category:data.error_category,safe_message:data.safe_message};updateProviderDisplay();toast(ok?`${providerText(provider)} hoạt động · ${model}`:`${providerText(provider)} · ${data.error_category}: ${data.safe_message}`,ok?"success":"error")}
  catch(error){const detail=safeErrorDetail(error.message,error.status,error.contentType);cfg.provider_status[provider]={status:"failed",model,error_category:"PROVIDER_ERROR",safe_message:detail};updateProviderDisplay();toast(`${providerText(provider)} · ${model}: ${detail}`,"error")}
...
async function confirmRenameConversation(){const c=pendingRenameConversation,title=$("#renameInput").value.trim();if(!c||!title)return;try{const r=await fetch(`/api/conversations/${encodeURIComponent(c.conversation_id)}`,{method:"PATCH",headers:{"Content-Type":"application/json"},body:JSON.stringify({title})});if(!r.ok)throw await responseError(r);if(currentConversationId===c.conversation_id)$("#chatTitle").textContent=title;closeRenameDialog();await loadConversations();toast("Đã đổi tên","success")}catch(e){toast("Không đổi tên được: "+safeErrorDetail(e.message,e.status,e.contentType),"error")}}
...
function markConversationActive(id){$$('.thread-card').forEach(card=>card.classList.toggle("active",card.dataset.id===id))}
function updateConversationSidebar(conversationId,message,{title=null,runId=null}={}){
  const item=conversationCache.find(entry=>entry.conversation_id===conversationId);if(!item)return false;
  if(title)item.title=title;item.last_preview=String(message||"").slice(0,120);item.updated_at=Math.floor(Date.now()/1000);item.turn_count=(item.turn_count||0)+1;item.run_count=(item.run_count||0)+(runId?1:0);
  const card=$$('.thread-card').find(entry=>entry.dataset.id===conversationId);if(!card)return false;card.querySelector('.thread-title').textContent=item.title;card.querySelector('.thread-main').title=`${item.last_preview}\n${item.turn_count||0} lượt`;
  markConversationActive(conversationId);return true
}

async function loadConversation(id){
  if(busy)return;const loadId=++conversationLoadSequence,started=performance.now();projectWorkspace=null;renderProjectWorkspaceIndicator();markConversationActive(id);messages.innerHTML='<div class="conversation-loading" aria-live="polite">Đang tải cuộc trò chuyện…</div>';clearContextFile();
  try{
    const response=await fetch(`/api/conversations/${encodeURIComponent(id)}`);if(!response.ok)throw new Error("not found");const conversation=await response.json();if(loadId!==conversationLoadSequence)return;
    currentConversationId=id;rememberConversation(id);projectWorkspace=conversation.project_workspace||null;renderProjectWorkspaceIndicator();app.classList.remove("mobile-side-open");updatePanelButtons();history=(conversation.messages||[]).map(m=>({role:m.role,content:m.content,context_sources:m.context_sources,sources:m.sources}));messages.innerHTML="";const turns=conversation.turns||fallbackTurns(conversation.messages),lastTurnIndex=turns.length-1;turns.forEach((turn,index)=>renderTurn(turn,conversation,index===lastTurnIndex));$("#chatTitle").textContent=conversation.title||"Cuộc trò chuyện";/* Persisted context is historical metadata; reload never activates it in the draft. */persistedContextSources=Array.isArray(conversation.context_sources)?conversation.context_sources:[];renderContextAttachments();if(conversation.provider&&cfg.available[conversation.provider]){$("#provider").value=conversation.provider;populateModels(conversation.model)}if(conversation.processing_mode){const restored=mapUiModeToChatStrategy(conversation.processing_mode)||"adaptive-auto";localStorage.setItem("adaptive.mode",restored);currentRequestedMode=restored;updateModeDisplay();renderModelPicker()}currentRunId=(conversation.run_ids||[]).at(-1)||null;$("#exportBtn").disabled=!currentRunId;markConversationActive(id);resetInspector("idle");scrollBottom();if(window.console?.debug)console.debug("conversation_switch_timing",{conversation_query_ms:Math.round(performance.now()-started),evidence_query_ms:null})
  }catch(error){if(loadId!==conversationLoadSequence)return;messages.innerHTML='<div class="conversation-load-error" role="alert">Không đọc được cuộc trò chuyện.</div>';toast("Không đọc được cuộc trò chuyện: "+error.message,"error")}
}
...
function compareAnswer(result,limit=250){if(!result)return"Chưa chạy";if(result.answer)return String(result.answer).replace(/\s+/g," ").trim().slice(0,limit);return result.error?`Không có câu trả lời · ${safeErrorDetail(result.error,result.http_status,result.content_type).slice(0,120)}`:"Không có câu trả lời"}
...
function contextSourceName(source){return sourceFilename(source)||"Tệp"}
function safeContextFilename(value,fallback="Tệp"){
  const raw=String(value||"").trim();if(!raw)return fallback;const normalized=raw.replace(/\\/g,"/"),absolute=/^(?:[a-z]:\/|\/+|file:|https?:)/i.test(normalized);if(absolute){const basename=normalized.split("/").filter(Boolean).at(-1);return basename||fallback}const safe=normalized.split("/").filter(part=>part&&part!=="."&&part!=="..").join("/");return safe||fallback;
}
function sourceFilename(source){return typeof source==="string"?source:source?.relative_path||source?.relativePath||source?.safe_path||source?.filename||source?.file_name||source?.name||""}
function renderProjectWorkspaceIndicator(){const indicator=$("#projectWorkspaceIndicator");if(!indicator)return;const workspace=projectWorkspace;if(!workspace?.project_id){indicator.hidden=true;indicator.innerHTML="";return}const files=Array.isArray(workspace.files)?workspace.files:[],paths=files.map(file=>safeContextFilename(file.relative_path||file.filename,"")).filter(Boolean);indicator.hidden=false;indicator.title=paths.join("\n");const projectName=workspace.name||"Dự án";indicator.innerHTML=`${svgIcon("folder","small")}<span class="project-workspace-name">${esc(projectName)}</span><span class="project-workspace-count">· ${Number(workspace.file_count||files.length)} tệp</span><span class="project-workspace-status">· Đang dùng</span><button type="button" class="project-workspace-detach" aria-label="Gỡ dự án khỏi ngữ cảnh ${esc(projectName)}" title="Gỡ khỏi ngữ cảnh">${svgIcon("close","small")}</button>`;indicator.querySelector("button")?.addEventListener("click",detachProjectWorkspace)}
function sourceIsExternal(source){
  if(typeof source==="string")return /^(?:https?:|file:)/i.test(source.trim());
  if(!source||typeof source!=="object")return false;
  const marker=[source.type,source.kind,source.source_type,source.origin].filter(Boolean).join(" ").toLowerCase();
  return Boolean(source.url||source.uri||source.link||/external|web|url|citation/.test(marker));
}
function knownContextSourceNames(extra=[]){
  const current=[...contextAttachments.values()].map(item=>item.source||item.filename),saved=persistedContextSources||[];
  return new Set([...current,...saved,...(Array.isArray(extra)?extra:[])].map(source=>safeContextFilename(sourceFilename(source),"" )).filter(Boolean));
}
function sourceEntries(sources,extra=[]){
  const known=knownContextSourceNames(extra);return (Array.isArray(sources)?sources:[]).map(source=>{
    const raw=sourceFilename(source);if(!raw)return null;const label=safeContextFilename(raw,"Nguồn"),hasFileIdentity=typeof source==="object"&&Boolean(source.filename||source.file_name);
    return {label,uploaded:!sourceIsExternal(source)&&(known.has(label)||hasFileIdentity)};
  }).filter(Boolean);
}
function activeContextForRequest(){return draftContextActive&&Boolean($("#context").value.trim())}
function contextSourcesForRequest(){if(!activeContextForRequest())return [];return [...contextAttachments.values()].filter(item=>item.status==="ready"&&item.source).map(item=>({...item.source}))}
function contextAttachmentPending(){return [...contextAttachments.values()].some(item=>item.status==="loading"||item.status==="processing")}
function projectStructure(paths){
  const root={};for(const path of [...paths].sort((a,b)=>a.localeCompare(b))){let node=root;for(const part of path.split("/")){node=node[part]||(node[part]={})}}
  const render=(node,depth=0)=>Object.keys(node).sort((a,b)=>a.localeCompare(b)).flatMap(name=>{const child=node[name],folder=Object.keys(child).length>0;return [`${"  ".repeat(depth)}${name}${folder?"/":""}`,...(folder?render(child,depth+1):[])]});return render(root).join("\n");
}
function rebuildContextFromAttachments(){
  ...
  const labels=ready.map(item=>safeContextFilename(contextSourceName(item.source),"Tệp"));
  const projectLike=ready.some(item=>Boolean(item.source.relative_path));
  $("#context").value=projectLike
    ? `[PROJECT STRUCTURE]\n${projectStructure(labels)}\n\n[RETRIEVED CONTEXT]\n\n${ready.map(item=>`SOURCE: ${safeContextFilename(contextSourceName(item.source),"Tệp")}\n${item.text}`).join("\n\n")}`
    : ready.map(item=>`===== ${safeContextFilename(contextSourceName(item.source),"Tệp")} =====\n${item.text}`).join("\n\n--- attached file ---\n\n");
}
function contextAttachmentStateText(state){
  if(state==="loading"||state==="processing")return "Đang xử lý";
  if(state==="error"||state==="unsupported")return "Lỗi";
  return "";
}
function renderContextAttachments(){
  const list=$("#contextFileList");
  if(!list)return;
  // Only current unsent files belong in the composer tray. Historical context
  // sources remain available to the request/evidence layer, never as draft UI.
  const items=[...contextAttachments.values()];
  const rows=items.map(item=>{const filename=safeContextFilename(sourceFilename(item.source)||item.filename,item.file?.name||item.filename||"Tệp"),stateText=contextAttachmentStateText(item.status),state=stateText?`<span class="context-file-state ${item.status}">· ${stateText}</span>`:"",retry=item.status==="error"||item.status==="unsupported"?'<button type="button" class="context-file-retry">Thử lại</button>':"";return `<div class="context-file-row ${item.status}" data-context-file-id="${esc(item.id)}" role="listitem"><div class="context-file-main"><span class="context-file-icon">${svgIcon("file","small")}</span><b title="${esc(filename)}">${esc(filename)}</b>${state}</div><div class="context-file-actions">${retry}<button type="button" class="context-file-remove" aria-label="Bỏ tệp ${esc(filename)}">×</button></div></div>`}).join("");
  list.innerHTML=rows;
  list.querySelectorAll(".context-file-row").forEach(row=>{const id=row.dataset.contextFileId,retry=row.querySelector(".context-file-retry");if(retry)retry.onclick=()=>retryContextFile(id);row.querySelector(".context-file-remove").onclick=()=>removeContextAttachment(id)})
}
...
async function processContextFile(file,id=null,relativePath=null){
  if(!file)return false;
  persistedContextSources=[];draftContextActive=true;
  const attachmentId=id||`context_${++contextAttachmentSequence}`,item=contextAttachments.get(attachmentId)||{id:attachmentId,file,filename:file.name,relativePath,status:"loading",text:"",source:null,error:""};item.file=file;item.filename=file.name;item.relativePath=relativePath??item.relativePath??null;item.status="loading";item.text="";item.source=null;item.error="";contextAttachments.set(attachmentId,item);activeContextFile=file;rebuildContextFromAttachments();renderContextAttachments();
  try{
    ...
    const response=await fetch("/api/context/prepare",{method:"POST",headers:{"Content-Type":"application/json"},body:JSON.stringify({filename:file.name,relative_path:item.relativePath||undefined,content_base64:await encodeFileBase64(file)})});
    ...
    item.status="ready";item.text=prepared.text;item.source=prepared.source;item.filename=safeContextFilename(prepared.source.relative_path||prepared.source.filename||item.filename,file.name||"Tệp");item.error="";rebuildContextFromAttachments();renderContextAttachments();toast(`Đã nạp ${prepared.source.relative_path||file.name} vào ngữ cảnh`,"success");return true
  }catch(error){
...
function retryContextFile(id){const item=contextAttachments.get(id);if(!item||["loading","processing"].includes(item.status))return;item.relativePath?processContextFile(item.file,id,item.relativePath):processContextFile(item.file,id)}
function removePersistedContextSource(id){
  const index=Number(String(id).slice(6));if(!Number.isInteger(index)||index<0||index>=persistedContextSources.length)return;
  const source=persistedContextSources[index],marker=`===== ${safeContextFilename(contextSourceName(source),"Tệp")} =====`,separator="\n\n--- attached file ---\n\n",current=$("#context").value,markerAt=current.indexOf(marker);
  if(markerAt<0&&persistedContextSources.length>1){toast("Không thể bỏ riêng tệp đã lưu này.","error");return}
  persistedContextSources.splice(index,1);
  if(markerAt<0){$("#context").value=""}else{const start=markerAt>0&&current[markerAt-1]==="\n"?markerAt-1:markerAt,separatorAt=current.indexOf(separator,markerAt+marker.length),end=separatorAt>=0?separatorAt+separator.length:current.length;$("#context").value=(current.slice(0,start)+current.slice(end)).replace(/\n{3,}/g,"\n\n").trim()}
  renderContextAttachments();
}
function removeContextAttachment(id){if(!contextAttachments.has(id))return;contextAttachments.delete(id);activeContextFile=[...contextAttachments.values()][0]?.file||null;rebuildContextFromAttachments();renderContextAttachments()}
function clearDraftAttachments(){activeContextFile=null;contextAttachments.clear();for(const id of ["#contextFile","#contextFolder"]){const input=$(id);if(input)input.value=""}const list=$("#contextFileList");if(list)list.innerHTML=""}
function clearContextFile(){clearDraftAttachments();persistedContextSources=[];draftContextActive=false;$("#context").value=""}
function closeAttachmentMenu(){const menu=$("#attachmentMenu"),trigger=$("#attachButton");menu?.classList.remove("open");menu?.setAttribute("aria-hidden","true");trigger?.setAttribute("aria-expanded","false")}
function folderRelativePath(file){const parts=String(file?.webkitRelativePath||"").replace(/\\/g,"/").split("/").filter(Boolean);return parts.length>1?parts.slice(1).join("/"):""}
function isProjectNoise(path){return path.split("/").some(part=>PROJECT_NOISE_DIRECTORIES.has(part))}
function supportedProjectFiles(files){const supported=new Set((cfg?.context_file_extensions||[]).map(extension=>String(extension).replace(/^\./,"").toLowerCase()));return files.map(file=>({file,path:folderRelativePath(file)})).filter(item=>item.path&&!isProjectNoise(item.path)&&supported.has(item.path.split(".").at(-1).toLowerCase()))}
async function importProjectFolder(files){const accepted=supportedProjectFiles(files);if(accepted.length>MAX_PROJECT_FILES){toast("Hỗ trợ tối đa 20 tệp mã nguồn trong một dự án.","error");return}if(!accepted.length){toast("Không tìm thấy tệp mã nguồn được hỗ trợ trong thư mục này.","error");return}clearDraftAttachments();persistedContextSources=[];$("#context").value="";for(const item of accepted.sort((a,b)=>a.path.localeCompare(b.path)))await processContextFile(item.file,null,item.path);const ready=[...contextAttachments.values()].filter(item=>item.status==="ready"&&item.source&&typeof item.text==="string");if(ready.length!==accepted.length){toast("Chưa thể tạo project workspace vì có tệp không xử lý được.","error");return}const rootName=String(accepted[0]?.file?.webkitRelativePath||"").replace(/\\/g,"/").split("/").filter(Boolean)[0]||"Dự án";try{const response=await fetch("/api/conversations/project-workspace",{method:"POST",headers:{"Content-Type":"application/json"},body:JSON.stringify({conversation_id:currentConversationId,name:rootName,files:ready.map(item=>({filename:item.source.filename,relative_path:item.source.relative_path,content:item.text}))})});if(!response.ok)throw await responseError(response);const data=await response.json();currentConversationId=data.conversation_id||currentConversationId;rememberConversation(currentConversationId);projectWorkspace=data.project_workspace||null;clearDraftAttachments();draftContextActive=false;$("#context").value="";renderProjectWorkspaceIndicator();await loadConversations();toast(`Đã gắn dự án ${rootName}.`,"success")}catch(error){toast("Không lưu được project workspace: "+safeErrorDetail(error.message,error.status,error.contentType),"error")}}
async function detachProjectWorkspace(){if(!currentConversationId||!projectWorkspace)return;try{const response=await fetch(`/api/conversations/${encodeURIComponent(currentConversationId)}/project-workspace`,{method:"DELETE"});if(!response.ok)throw await responseError(response);projectWorkspace=null;renderProjectWorkspaceIndicator();toast("Đã tách dự án khỏi cuộc trò chuyện.","success")}catch(error){toast("Không tách được dự án: "+safeErrorDetail(error.message,error.status,error.contentType),"error")}}
...
$("#attachButton").onclick=()=>{const menu=$("#attachmentMenu"),open=!menu.classList.contains("open");closeAttachmentMenu();if(open){menu.classList.add("open");menu.setAttribute("aria-hidden","false");$("#attachButton").setAttribute("aria-expanded","true");$("#chooseFiles").focus()}};$("#chooseFiles").onclick=()=>{closeAttachmentMenu();$("#contextFile").click()};$("#chooseFolder").onclick=()=>{closeAttachmentMenu();$("#contextFolder").click()};$("#contextFile").onchange=async event=>{const files=[...event.target.files];if(files.length){$("#context").value="";await Promise.all(files.map(file=>processContextFile(file)))}event.target.value=""};$("#contextFolder").onchange=async event=>{const files=[...event.target.files];if(files.length)await importProjectFolder(files);event.target.value=""};
