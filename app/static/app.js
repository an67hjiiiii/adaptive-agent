...
let contextAttachments = new Map(), contextAttachmentSequence = 0, persistedContextSources = [], draftContextActive = false;
...
async function loadConversation(id){
  if(busy)return;const loadId=++conversationLoadSequence,started=performance.now();markConversationActive(id);messages.innerHTML='<div class="conversation-loading" aria-live="polite">Đang tải cuộc trò chuyện…</div>';clearContextFile();
  try{
...
async function importProjectFolder(files){const accepted=supportedProjectFiles(files);if(accepted.length>MAX_PROJECT_FILES_V11){toast("V1.1 hỗ trợ tối đa 20 tệp mã nguồn trong một dự án.","error");return}if(!accepted.length){toast("Không tìm thấy tệp mã nguồn được hỗ trợ trong thư mục này.","error");return}clearDraftAttachments();persistedContextSources=[];$("#context").value="";for(const item of accepted.sort((a,b)=>a.path.localeCompare(b.path)))await processContextFile(item.file,null,item.path)}
