...
## Multi-Agent versus Multi-Model scope

This project studies adaptive orchestration in a **homogeneous Multi-Agent LLM
system**. Multi-Agent means that a run may contain multiple bounded agent roles
or executions; it does **not** require multiple underlying models. Multiple
agents may use the same underlying Model.

The primary experiment deliberately holds the same **Provider**, **Model**, and
**model settings** fixed across Single, Fixed, Static, and Adaptive, and across
all LLM-backed agent roles in a comparison. This isolates the effect of
orchestration. If, for example, Planner used Gemini, Worker used DeepSeek, and
Verifier used Groq, a result could differ because of model capability rather
than orchestration. That is a confounding factor — *yếu tố gây nhiễu*: không
biết kết quả khác do điều phối hay do Model.

Heterogeneous Multi-Agent/Multi-Model routing (for example, separate Gemini,
DeepSeek, and Groq agents) is outside the primary experiment. It may be studied
later as a separate research question or an optional robustness experiment
after Main, but it is not added to the current Pilot or Main design.
...
- **Agent Execution** — one bounded role execution recorded by the controller;
  it is not a count of distinct models. One Model may back many Agent
  Executions, and retries remain governed by the call/request definitions below.
